...
function imgOrEmoji(filename, emoji, extraClass, altText){
  extraClass = extraClass || '';
  altText = escapeHtml(altText || '');
  var resolved = resolvedImage(filename);
  var src = escapeHtml(img(resolved));
  return '<img src="'+src+'" alt="'+altText+'" loading="lazy" class="'+extraClass+'" '+
...
function imgOrEmojiInitials(filename, initials, altText){
  altText = escapeHtml(altText || '');
  var resolved = resolvedImage(filename);
  var src = escapeHtml(img(resolved));
...
// ── DATA ─────────────────────────────────────────
// Hero background slideshow — your original "hero-background.jpg" stays
// exactly as it was and is the first slide; the rest are numbered 1 to 7.
// Add or remove filenames here any time — no limit on how many.
...
function renderDestinations(){
  ...
  g.innerHTML = DESTINATIONS.map(function(d){
    return '<div class="dest-card" data-dest="'+d.id+'">'+
      '<div class="dest-card-bg" style="background:'+d.color+'">'+imgOrEmoji(d.image,d.emoji,'',d.name)+'</div>'+
...
// ── STRUCTURED DATA FOR PACKAGES (SEO) ─────────────────────
// Generates Product/Offer JSON-LD for every safari package, using real data
// already in the PACKAGES array (name, description, image, price, currency).
// This makes packages eligible for Google to show price info directly in
// search results.
//
// Deliberately does NOT include AggregateRating/Review markup here. Google's
// structured-data guidelines require rating markup to reflect real,
// verifiable reviews collected for that specific item — fabricating numbers
// (or reusing a generic site-wide rating on every package) is a policy
// violation that can trigger a manual action against the whole site. Add
// this back in honestly once there's a real per-package review system.
function renderPackagesStructuredData(){
  var siteUrl = 'https://mothernaturesafaris.com/';

  var itemListElements = PACKAGES.map(function(p, i){
    ...
    return {
      ...
      'item': {
        ...
        'name': p.name,
        'description': p.desc,
        'image': img(resolvedImage(p.image)),
        'url': siteUrl + '#' + p.id,
  ...
  var schema = {
    ...
    '@type': 'ItemList',
    'name': 'Mother Nature Safaris — Safari Packages',
    'itemListElement': itemListElements
  };

  var existing = document.getElementById('packagesSchema');
  if (existing) existing.remove();
  var scriptTag = document.createElement('script');
  scriptTag.type = 'application/ld+json';
  scriptTag.id = 'packagesSchema';
...
function renderPackages(){
  ...
  g.innerHTML = filtered.map(function(p){
    return '<a class="pkg-card" href="packages/'+slugify(p.name)+'.html" data-pkg-id="'+p.id+'">'+
      '<div class="pkg-img" style="background:linear-gradient(135deg,#2C1810,#'+randomDark()+')">'+
        imgOrEmoji(p.image,p.emoji,'',p.name)+
      ...
      '<div class="pkg-footer">'+
        ...
        '<span class="btn-book-pkg">View Details</span>'+
      '</div>'+
    '</a>';
  }).join('');
}
...
// Must produce identical output to slugify() in build-package-pages.js —
// this is how a package card's link ("packages/<slug>.html") stays in sync
// with the actual filename the build script generates for that package.
...
function renderGallery(){
  ...
  g.innerHTML = GALLERY_ITEMS.map(function(item){
    return '<div class="gallery-item" style="background:'+item.color+'">'+
      imgOrEmoji(item.image,item.emoji,'',item.label)+
  ...
  g.innerHTML = TESTIMONIALS.map(function(t){
    return '<div class="testi-card">'+
      ...
      '<div class="testi-author">'+
        '<div class="testi-av">'+imgOrEmojiInitials(t.image,t.initials,'Photo of '+t.name)+'</div>'+
  ...
  host.innerHTML = '<div class="hero-bg-slides">' +
    slides.map(function(filename, i){
      return '<div class="hero-bg-slide' + (i === 0 ? ' active' : '') + '">' +
        '<img src="' + escapeHtml(img(filename)) + '" alt="East Africa safari landscape" loading="' + (i === 0 ? 'eager' : 'lazy') + '" ' +
...
// How long a visitor must wait between booking submissions from this browser.
// This is a courtesy/accident-prevention measure (double-taps, impatient
// re-clicks) — NOT a real anti-abuse control, since anyone can bypass
// client-side JS entirely and post directly to the Formspree endpoint.
// Genuine bot/spam protection has to happen server-side, via Formspree's
// own submission cap and optional CAPTCHA step.
var BOOKING_COOLDOWN_MS = 20000; // 20 seconds
var BOOKING_COOLDOWN_KEY = 'mns_last_booking_submit';
...
function submitBooking(){
  ...
  try {
    var lastSubmit = sessionStorage.getItem(BOOKING_COOLDOWN_KEY);
    if (lastSubmit && (Date.now() - Number(lastSubmit)) < BOOKING_COOLDOWN_MS){
      var waitSec = Math.ceil((BOOKING_COOLDOWN_MS - (Date.now() - Number(lastSubmit))) / 1000);
      showToast('Please wait ' + waitSec + 's before submitting again.', 'error');
      return;
    }
  } catch (e) { /* sessionStorage unavailable — just skip the cooldown check */ }
  ...
  try { sessionStorage.setItem(BOOKING_COOLDOWN_KEY, String(Date.now())); } catch (e) { /* non-fatal */ }
...
function refreshImages(){
  document.getElementById('navLogoIcon').innerHTML = imgOrEmoji('logo.png','🌿','','Mother Nature Safaris logo');
  document.getElementById('footerBrandTitle').innerHTML = imgOrEmoji('logo.png','🌿','','Mother Nature Safaris logo') + ' Mother Nature Safaris';
  document.getElementById('aboutImgFrame').innerHTML = imgOrEmoji('about-photo.jpg','🐘','','Mother Nature Safaris guide with wildlife on safari');
  renderHeroSlideshow();
  renderDestinations();
...
function initSite(){
  ...
  // A package page's "Book This Safari" button links here as
  // index.html?package=<name>#contact — pre-select the matching option in
  // the booking form's package dropdown so the handoff feels continuous
  // rather than dumping the visitor into a blank form.
  var packageParam = new URLSearchParams(window.location.search).get('package');
  if (packageParam){
    var pkgSelect = document.getElementById('fPackage');
    if (pkgSelect){
      var target = packageParam.substring(0, 8).toLowerCase();
      for (var i = 0; i < pkgSelect.options.length; i++){
        if (pkgSelect.options[i].text.toLowerCase().indexOf(target) !== -1){
